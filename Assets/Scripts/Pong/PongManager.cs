...
public class PongManager : MonoBehaviour
{
    ...
    public void Score(BallController ball, string wallID, BallTypes type)
    {
        ...
        {
            PlayerScore++;
            managerObject.SendMessage("AddPB", (double)(((double)ball.currentValue) * (1 + baseBall.stacks * baseBall.increaseValue)));
        }
        else
        {
            EnemyScore++;
            if(EnemyScore >= 5 + (scoreLimit.stacks + scoreLimitAdjust) * scoreLimit.increaseValue)
                managerObject.SendMessage("AddPB", (double)(((double)ball.currentValue) * (1 + baseBall.stacks * baseBall.increaseValue) * 1.25));
            else
                managerObject.SendMessage("AddPB", (double)(((double)ball.currentValue) * (1 + baseBall.stacks * baseBall.increaseValue) + 1));
        }
